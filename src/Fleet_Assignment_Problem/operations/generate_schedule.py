...
for ac_idx in range(MAX_AIRCRAFT):
...
df_schedule = pd.DataFrame(schedule).sort_values(by="Dept Time").reset_index(drop=True)
OUTPUT_FILE = Path(__file__).resolve().parents[3] / "data" / "raw" / "Flight_Schedule" / "schedule_fap.csv"
